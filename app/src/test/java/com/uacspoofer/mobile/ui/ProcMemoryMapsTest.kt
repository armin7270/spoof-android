...
class ProcMemoryMapsTest {
    ...
    fun classifiesTorAndXrayLibraries() {
        ...
        assertEquals("tun2socks", ProcMemoryMaps.classifyMapping("/data/app/libtun2socks.so"))
        assertEquals("pow", ProcMemoryMaps.classifyMapping("/data/app/libaether.so"))
        assertEquals("pow", ProcMemoryMaps.classifyMapping("/data/app/libgopsi.so"))
